...
╚══════╝ ╚═════╝    ╚═╝    ╚═════╝ ╚═╝     ╚═╝  ╚═╝ ╚══════╝ ╚═╝╚══════╝
    ...
    print("Calcule equivalência e probabilidades (15 a 20 dezenas)")
    print("=" * 70)
    print()
...
def hypergeom_p(n_chosen: int, hits: int) -> float:
    """
    P(X = hits) onde X = quantidade de acertos (interseção) entre:
    - n_chosen números escolhidos pelo jogador
    - 15 números sorteados de um total de 25
    """
    if hits < 0:
        return 0.0
...
def p_at_least_one(p_single: float, trials: int) -> float:
    """
    Probabilidade de ocorrer >=1 sucesso em 'trials' tentativas independentes:
    1 - (1-p)^trials (com estabilidade numérica).
    """
    if trials <= 0:
        return 0.0
    if p_single <= 0.0:
...
def run_once() -> None:
    ...
    # Probabilidades por faixa (acertos 11 a 15)
    ...
    for hits in [15, 14, 13, 12, 11]:
        ...
        prob_rows.append([
            faixa,
            format_odds_from_p(p1),
            f"{format_odds_from_p(pn)}  ({format_float_br(pn * 100, 4)}%)",
        ])
...
def main() -> None:
    while True:
        ...
        choice = input("Enter para novo cálculo | 'q' para sair: ").strip().lower()
